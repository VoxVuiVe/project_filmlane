...
@WebServlet(urlPatterns = { "/index", "/favourites", "/history"})
public class HomeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String path = req.getServletPath(); // localhost:8080/asm-java4/login >>> path = */login*
		switch (path) {
		case "/index":
			doGetIndex(req, resp);
			break;
		case "/favourites":
			doGetFavourites(session, req, resp);
			break;
		case "/history":
	...
	private void doGetFavourites(HttpSession session, HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.setAttribute("videos", videos);
		req.getRequestDispatcher("/views/user/movies-favourite.jsp").forward(req, resp);
	}
	...
	private void doGetHistory(HttpSession session, HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		histories.forEach(item -> videos.add(item.getVideo()));
		req.setAttribute("videos", videos);
		req.getRequestDispatcher("/views/user/movies-history.jsp").forward(req, resp);
	}
}
